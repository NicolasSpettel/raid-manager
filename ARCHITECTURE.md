...
`dotnet build -warnaserror` + `dotnet test` green (116 tests).
**Next (world):** the FM **condition/stamina** cost of raiding hard (now buildable on the wired-in attributes),
unifying the two raider models (M1 `RaiderRecord` vs world `Raider`), then contracts/transfers + the career ledger.
