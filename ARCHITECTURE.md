...
(§3) → the **home hub** (`HomeShell`: 10 nav tabs, 6 built — incl. **Transfers**), whose Squad/market raiders open a **unit page**.
...
retire and leave the roster — the career runs season after season. Clock + lockout + season number persist.
**Recruitment fills those gaps (GDD §8b/§10, first slice):** the **Transfers tab** offers your **youth
academy's** intake (`YouthProgram` — a deterministic pool of young prospects, refreshed each season, promotable
for a nominal fee) and the **transfer market** (`TransferMarket` — the living world's ~400 free agents, real
inspectable characters, priced best-first, signable for a performance-driven fee). Async negotiation / scouting
fog / poaching are later layers.
`dotnet build -warnaserror` + `dotnet test` green (169 tests).
**Next (world):** the **Inbox** (GDD §11) surfacing the week's events, retirements and offers; deepening
transfers (async negotiation, scouting fog, poaching, wages/contracts); balancing the load/injury/growth rates;
and richer activities (professions/quests/drills).
...
| `src/Game` | The management layer: `GuildSave` aggregate (guild + roster + gear + injuries + economy + raid history), `SaveMigrations` (v4, real chained v1→v2→v3→v4), `SaveSerializer`, atomic `FileStorageAdapter`, `SaveService`, `Guilds.CreateStarter`, `Warband` (projects raiders → combatants, folding gear + injury penalty + `CombatResolution` of attributes), `CombatResolution` (§8a′ attribute→combat bridge), `ConditionModel` (FM two-axis freshness/sharpness + performance fold), `Campaign` (headless N-raid loop), `TransferMarket` (§8b — sign living-world free agents) + `YouthProgram` (§10 — academy intake, promote prospects), `Aging` (§8 — season rollover: develop/decline/retire), `RaidResolver` (folds a fight into gold / loot / injuries + a `RaidSummary`), `Manager`/`Managers` (§2) + `JobMarket` (§4 offers + world-guild→save conversion) + `Negotiations` (§4 contract talk) + `GuildLore` (§3 backstory), and **`World/`** — the composition entity model (`Raider` = identity/vocation/attributes/condition components), the deterministic `WorldGen` latent-factor pipeline (~6,500 characters from a seed), `Ratings` (stars derived on read), `WorldText` (canonical serialize + golden hash), `SeasonRace` (the whole world races the season `SeasonRaid` in parallel → a ranked global/regional leaderboard, calibrated to the locked pacing), and `Chronicle` (folds a season into a permanent record — champion, world-firsts, top-100); plus **`Season/`** — the player week loop (`SeasonCalendar`, `WeekStance`/`WeekPlan`+`ActivityPlan` for "plan my week", `Lockout` for the weekly reset, `WeekRunner` raiding the ladder, `SeasonSchedule` = the calendar of events, `WeekSchedule`+`WeekPlanner` = the granular week plan (5-man groups, training), `WeekExecutor` = runs a planned week per-raider, `WeeklyActivities.Train`, `Injuries`, `MoraleModel`, `DayExecutor` = one day of the 4-slot calendar) | App · Godot |
| `src/Sim` | Headless CLI `run dummy --seed N` → the real Engine; future golden/probe/campaign home | App · Godot |
| `src/App` | Godot 4.7 C#: `AppTheme` (carved-stone `Theme` + procedural stone backdrop), `Main` coordinator now starting at the **opening screen** (`WelcomeView` §1: Continue/New Career/Quit) → **`ManagerCreationView`** (§2: identity + background + 7-attribute point-buy) → **`JobOffersView`** (§4: pick a real low-prestige guild from the generated world — roster/finances/expectation/rival) → **`NegotiationView`** (§4 contract talk: push gently/hard, sign or walk) → **`GuildIntroView`** (§3: the guild's past + your brief) → **`HomeShell`** — the home hub: a persistent nav bar (10 FM-style tabs) over a content area. Built: **Home** (dashboard) / **Squad** (`RosterView`; raiders clickable → **`RaiderView`** unit page) / **Calendar** (season events) / **Guild** (finances/brief/history) / **Manager** (profile). Placeholder (labelled with what they'll be): Tactics / Transfers / Staff / Inbox / World. Raids + the raider page are full-screen and return to the hub. → `CombatView`/`StageView` playback → save. Both combat views are pure consumers of the same event stream. **The only Godot-referencing project.** Run headless-free with `.\run-game.ps1`. | — |
