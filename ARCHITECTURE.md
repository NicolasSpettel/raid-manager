...
**Condition/stamina (§8 [LOCKED], economy-model §1):** `ConditionModel` runs the FM two-axis system —
raiding drains **Freshness** (recovered on rest ∝ Endurance) and builds **Sharpness** (lost on the bench);
low condition folds into combat as worse output. **Off-day activities (§6)** make grinding pay: `WeeklyActivities`
runs **dungeons** (a gear catch-up faucet) and **training** (targeted attribute development), and `Injuries`
rolls condition-driven injuries (fatigue + heavy load → knocks). So the whole week is a real dial:
`sim play --stance grind` gears fast (62→347) and trains stats but batters the roster (fatigue + injuries),
while `--stance relax` stays fresh and healthy but progresses slower.
`dotnet build -warnaserror` + `dotnet test` green (130 tests).
**Next (world):** balance the grind↔rest dial (first-pass rates make grind roughly break-even — should be a
viable-but-risky push), Morale as its own axis, contracts/transfers, and collapsing the two raider models into one type.
