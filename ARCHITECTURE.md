...
Run them with `sim world --seed N` / `sim season --seed N`. `dotnet build -warnaserror` + `dotnet test` green (107 tests).
**Next (world):** the player-facing **calendar/week loop** (raid days, weekly lockout, the visual timeline),
then contracts/transfers + the raider career ledger (seed+delta persistence), and multi-season aging.
