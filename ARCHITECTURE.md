...
low condition folds into combat as worse output. The tradeoff emerges: `sim play --stance grind` burns the
roster out (freshness → 0, stalls), while `--stance relax` stays fresh and gets *further*.
`dotnet build -warnaserror` + `dotnet test` green (122 tests).
**Next (world):** balance the grind↔rest dial (grind shouldn't be strictly dominated), Morale as its own
axis + condition-driven injuries, then contracts/transfers, and finally collapsing the two raider models into one type.
