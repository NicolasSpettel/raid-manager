...
Run them with `sim world` / `sim season` / `sim play --stance grind`. `dotnet build -warnaserror` + `dotnet test` green (113 tests).
**Next (world):** the FM **condition/stamina** cost of raiding hard (needs the entity/attribute model wired
into combat, §8a′), then contracts/transfers + the raider career ledger, and the player joining the world race.
