...
| 2026-07-06 | **M1 build — economy/progression (first-pass numbers):** raid gold 500 win / 100 wipe; XP = 100 base + contribution, level-up curve 100×level; loot auto-equips-if-upgrade (Power score); recruitment flat 400 gold. All placeholder for the priced economy-model.md pass. | M1 BUILD (assistant) — tunable |
...
| 2026-07-06 | **Levels now affect combat (was a no-op).** `Level` was awarded/displayed but never read in combat — progression was gear-only. Fixed: +8%/level (past 1) to HP + auto-attack in `Warband.ToCombatant` (first-pass, tunable). Gap: ability amounts are still flat (no spellpower stat), so this favours tanks/melee over casters/healers. | BUILD (assistant) — tunable |
| 2026-07-06 | **Frostwarden validated as endgame, not broken.** A fresh guild is 0/6 on it at every tier, which looked alarming; proved (regression test) that a guild which farms the Ashen King (→ Lv 8, gear ~408) then clears **Frostwarden Normal 6/6**. So it's correctly gated behind progression, not dead content. Confirm the intent (should it need Heroic-tier gear?) at a real balance pass. | BUILD (assistant) — verified |
