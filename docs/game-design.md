...
| 2026-07-06 | **Front-end started top-down (dev-directed: "build the interface more; start from the top — welcome screen, then manager creation").** `src/App` now opens on a **welcome screen** (§1: Continue [loads the save] / New Career / Quit; Load Career/Hall of Fame/Settings stubbed) → **manager creation** (§2: name/age/region, one of 4 **backgrounds** that seed an attribute spread + perk, and a **7-attribute point-buy** with live totals) → the roster (now showing the manager). `ManagerProfile` (Content) = the 7 attributes + 4 backgrounds; `Manager`/`Managers.Create` (Game); persisted on `GuildSave.Manager` (nullable, additive — no migration). Run the game with `.\run-game.ps1` (no editor). | UI BUILD (dev-directed) |
| 2026-07-06 | **Getting a job — the offer screen built (dev-directed: "choose from some guilds, with info about them").** New Career now generates this career's **living world** (`WorldGen`) and `JobOffersView` (§4) shows ~8 **struggling Local-tier** guilds to inspect — name/region/tier, roster size + avg stars, **finances** (bank in the economy-model §2 local band + weekly wage bill), the **board's expectation**, and a **regional rival**. `JobMarket` (Game): `OffersFor` builds the offers deterministically; `Take` converts the chosen world `Guild` into the playable `GuildSave` — its real roster mapped world `Raider` → `RaiderRecord` with **attributes + condition carried over** (the shared-component payoff), finances from tier. So you join a real guild in the world, not a generic starter. **Contract negotiation** (§4 back-and-forth: salary/length/budget, accept/counter AI) is still deferred — for now you just take the job. | JOB-OFFERS BUILD (dev-directed) |
| 2026-07-07 | **Negotiation + guild intro + raider unit page (dev-directed).** Contract talk (§4), simple two-button as asked: `Negotiations` (Game) — Start sets terms (salary + a board **expectation level**); Push (gentle/hard) either softens them (lower expectation, more pay), makes them hold ("take it or leave it" once patience is spent), or — on a hard push, especially with weak Negotiation — **pulls the offer**. Sign or walk. `NegotiationView` drives it. On signing → `GuildIntroView` (§3): the guild's seeded **backstory** (`GuildLore` — founding, motto, best-ever finish, a legendary ex-raider) + your agreed brief, then "take the reins" → the roster (home), which now saves the negotiated expectation on `GuildInfo.BoardExpectation`. Roster raiders are **clickable → `RaiderView`**: the base stats — class/role, the 11 attributes as bars, freshness/sharpness/morale (gear/stars once wired). Numbers first-pass. | UI BUILD (dev-directed) |
| 2026-07-07 | **Home hub with tab navigation (dev-directed: "the home screen with navigation to all the different tabs").** `HomeShell` (App): a persistent nav bar (Home · Squad · Calendar · Guild · Manager, + Save/Menu) over a swappable content area. **Home** = dashboard (squad size, board brief, next calendar event, last raid); **Squad** embeds the `RosterView` (raid/rest/recruit; raiders → unit page); **Calendar** = the season's upcoming events (preview; interactive week-play is the next step); **Guild** = reputation/bank/brief + recent raid history; **Manager** = your profile (age/region/background + the 7 attributes as bars). Actions that leave the hub (raids, the raider page) are full-screen and return to it. `Main` now routes career entry/continue → Home, in-hub actions → the Squad tab. Later expanded to the full **10-tab** set (5 built + Tactics/Transfers/Staff/Inbox/World as labelled placeholders). | UI BUILD (dev-directed) |
| 2026-07-07 | **Architecture: unified raider model + world/season persistence (dev-directed).** (1) Collapsed the two raider types into one `RaiderRecord` (entities §2 composition) — the world `Raider` is gone; a world raider IS a guild raider, so `JobMarket.Take` carries attributes/condition/identity/vocation with no conversion. World-gen golden byte-identical. (2) ADR-0007 first slice: a career now **persists its living world** — the save pins `WorldSeed` + `GeneratorVersion` + `ManagerGuildId` + `SeasonWeek` and materialises the player's guild (the hot ~30 units); on load the world regenerates byte-identically from the seed (Continue rebuilds it), so a career stays in the *same* world across sessions. Deferred: the background **delta log** (transfers/aging), season-boundary **compaction**, the **Chronicle** save, lazy background materialisation. | ARCH BUILD (dev-directed) |
| 2026-07-07 | **Interactive Calendar / week planner (dev-directed: "calendar").** The Calendar tab is now playable: it shows the current week + upcoming events + any holiday, a **per-day planner** (7 dropdowns: Raid/Dungeon/Training/Rest) and an **Advance the week** button. Advancing runs the real headless season loop (`WeekExecutor`: raids `Encounters.All`, dungeons drop gear to a 5-man group, training develops the weakest, per-raider freshness/sharpness/morale/injury), updates the guild, increments + persists `SeasonWeek`, and shows a one-line week summary. `WeekPlanner.FromDays` builds the schedule from the per-day choices; the season ends at week 12 (new-season rollover + aging still to come, and the granular 5-man-group assignment UI). | UI BUILD (dev-directed) |
| 2026-07-07 | **4-slot calendar grid + day/week simulation (dev-directed: "vertical calendar, each day has 4 slots, choose an activity, simulate the day or the week").** Replaced the per-day dropdowns with a proper grid: 7 day rows, each with **4 slot-buttons** (the §6 noon/afternoon/evening/night slots) that cycle Rest→Raid→Dungeon→Training on click; past days locked, current day highlighted. **Simulate day** / **Simulate week** run the new day-granular `DayExecutor` (a raid slot → one raid night on the ladder under the weekly lockout; dungeon slots → catch-up gear; training → develop the weakest; per-raider `ConditionModel.AfterDay` freshness/sharpness + a daily injury roll from how busy the day was). The save now tracks `SeasonDay` (0–6) and `DownedThisWeek` (the weekly lockout, reset on the Monday rollover), both persisted, so a career's clock survives save/load. First-pass daily rates; morale-on-the-day-loop and per-slot individual assignment still to come. | UI BUILD (dev-directed) |
