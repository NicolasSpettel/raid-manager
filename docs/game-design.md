...
| 2026-07-06 | **M1 build — combat depth (first-pass, tunable):** mechanic archetypes implemented = spreadDamage, tankBuster, enrage, raidDot, tankDebuff (a stacking damage-taken aura = the tankSwapDebuff idea minus the taunt-swap). Aura system = DoTs + stacking debuffs. interruptibleCast + threat/taunt-swaps still deferred. | M1 BUILD (assistant) — tunable |
